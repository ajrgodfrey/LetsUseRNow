...
This chapter was created using version 0.26.0 of the \Rpkg{BrailleR} package. This version of the package has a number of other packages that it depends on for functionality. When you make the \Rpkg{BrailleR} package available, you may see a few introduction messages that tell you about the other packages being loaded. You will also be told about some default settings for the package that can be altered later to suit your needs. 
...
See the help page for the \Rcmd{txtOut} command as it includes the history of the command and its relatives. The other related commands add comments to the text file while \Rcmd{txtStop} stops the processing entirely. Also note that the help page mentions how to get the packages needed for other file formats. 
...
\end{Schunk}
% filename and.Rnw 
 and % filename VI018.Rnw 
\begin{Schunk}
\begin{Sinput}
> VI(dotplot(airquality$Wind, xlab="Average wind speed (mph)")) 
\end{Sinput}
\begin{Soutput}
Warning in dotplot.default(airquality$Wind, xlab = "Average wind speed (mph)"): The dotplot command is a wrapper for stripchart, and is not a base R function.
\end{Soutput}
...
%filename VIMakeAccessGraphs.Rnw
\begin{Schunk}
\begin{Sinput}
> attach(airquality)
